...
Azure Consumer Demand Forecasting Project
...
# ─────────────────────────────────────────────
st.set_page_config(
    page_title="Azure Demand Forecast",
    page_icon="☁️",
    layout="wide",
...
<style>
  ...
  /* Show hand cursor on all dropdowns and interactive widgets */
  .stSelectbox div[data-baseweb="select"] { cursor: pointer !important; }
  .stSelectbox div[data-baseweb="select"] * { cursor: pointer !important; }
  .stMultiSelect div[data-baseweb="select"] { cursor: pointer !important; }
  .stMultiSelect div[data-baseweb="select"] * { cursor: pointer !important; }
  .stSlider [role="slider"] { cursor: grab !important; }
  .stRadio label { cursor: pointer !important; }
  .stCheckbox label { cursor: pointer !important; }
  .stButton button { cursor: pointer !important; }
  .stDownloadButton button { cursor: pointer !important; }
  .stFileUploader label { cursor: pointer !important; }
  .stDateInput input { cursor: pointer !important; }
</style>
...
<div class="dashboard-header">
  <div class="header-left">
    ...
    <p>&nbsp;&nbsp;&nbsp;&nbsp; Forecast Integration & Capacity Planning &nbsp;·&nbsp; {status_html}</p>
  </div>
...
# ─────────────────────────────────────────────
# KPI ROW 2 — Operational Columns
# ─────────────────────────────────────────────
avg_cost = df['cost_usd'].mean() if 'cost_usd' in df.columns else 0
avg_av = df['availability_pct'].mean() if 'availability_pct' in df.columns else 0
hol_cnt = (df['is_holiday'] == 1).sum() if 'is_holiday' in df.columns else 0
avg_eco = df['economic_growth_index'].mean() if 'economic_growth_index' in df.columns else 0
avg_mkt = df['marketing_index'].mean() if 'marketing_index' in df.columns else 0
avg_it = df['it_spending_growth'].mean() if 'it_spending_growth' in df.columns else 0

st.markdown('<div class="section-title">💡 Operational & External Indicator KPIs</div>', unsafe_allow_html=True)
c1,c2,c3,c4,c5,c6 = st.columns(6)

with c1:
    st.markdown(kpi_card("💰", "Avg Cost (USD)", f"${avg_cost:.2f}", "cost_usd column", "neutral", "#ffa657"), unsafe_allow_html=True)
with c2:
    st.markdown(kpi_card("✅", "Avg Availability", f"{avg_av:.2f}%", "availability_pct column", "neutral", "#3fb950"), unsafe_allow_html=True)
with c3:
    st.markdown(kpi_card("🏖️", "Holiday Records", f"{hol_cnt:,}", "is_holiday = 1", "neutral", "#7ecfff"), unsafe_allow_html=True)
with c4:
    st.markdown(kpi_card("📉", "Economic Growth", f"{avg_eco:.3f}", "economic_growth_index avg", "neutral", "#a371f7"), unsafe_allow_html=True)
with c5:
    st.markdown(kpi_card("📣", "Marketing Index", f"{avg_mkt:.3f}", "marketing_index avg", "neutral", "#f78166"), unsafe_allow_html=True)
with c6:
    st.markdown(kpi_card("💻", "IT Spending", f"{avg_it:.3f}", "it_spending_growth avg", "neutral", "#0078d4"), unsafe_allow_html=True)
...
with tab3:
    if 'service_type' not in df.columns:
        ...
        if 'cost_usd' in df.columns:
            ...
            fig_cost.update_layout(**PLOTLY_THEME, height=380,
...
with tab4:
    ...
    if os.path.exists("rmse_history.csv"):
        ...
        fig_rh.update_layout(**PLOTLY_THEME, height=280,
...
# ══════════════════════════════════════════
# TAB 6 — cost_usd, availability_pct, is_holiday, provisioned_capacity
# ══════════════════════════════════════════
with tab6:
    col1,col2=st.columns(2)
    with col1:
        if 'cost_usd' in df.columns and 'timestamp' in df.columns:
            ct=df.set_index('timestamp')['cost_usd'].resample('D').mean().reset_index()
            fig=go.Figure(go.Scatter(x=ct['timestamp'],y=ct['cost_usd'],
                fill='tozeroy',fillcolor='rgba(255,166,87,.1)',
                line=dict(color='#ffa657',width=2),name='Avg Daily Cost (USD)'))
            fig.update_layout(**PLOTLY_THEME,height=380,
                title=dict(text="Daily Avg Cost (USD) — cost_usd",font=dict(size=13,color='#c8d8f0')))
            st.plotly_chart(fig,use_container_width=True)

    with col2:
        if 'availability_pct' in df.columns and 'timestamp' in df.columns:
            at=df.set_index('timestamp')['availability_pct'].resample('D').mean().reset_index()
            fig=go.Figure(go.Scatter(x=at['timestamp'],y=at['availability_pct'],
                fill='tozeroy',fillcolor='rgba(63,185,80,.08)',
                line=dict(color='#3fb950',width=2),name='Avg Availability %'))
            fig.add_hline(y=99,line_dash='dash',line_color='#f8a100',annotation_text='99% SLA')
            fig.update_layout(**PLOTLY_THEME,height=380,
                title=dict(text="Daily Avg Availability % — availability_pct",font=dict(size=13,color='#c8d8f0')))
            st.plotly_chart(fig,use_container_width=True)

    col3,col4=st.columns(2)
    with col3:
        if 'is_holiday' in df.columns:
            hol=df.groupby('is_holiday')['usage_units'].mean().reset_index()
            hol['label']=hol['is_holiday'].map({0:'Non-Holiday',1:'Holiday'})
            fig=go.Figure(go.Bar(x=hol['label'],y=hol['usage_units'].round(2),
                marker_color=['#0078d4','#f78166'],
                text=hol['usage_units'].round(1),texttemplate='%{text}',
                textposition='outside',textfont=dict(color='#c8d8f0')))
            fig.update_layout(**PLOTLY_THEME,height=380,
                title=dict(text="Avg Usage: Holiday vs Non-Holiday — is_holiday",font=dict(size=13,color='#c8d8f0')))
            st.plotly_chart(fig,use_container_width=True)

    with col4:
        if 'provisioned_capacity' in df.columns and 'timestamp' in df.columns:
            pc=df.set_index('timestamp')['provisioned_capacity'].resample('W').mean().reset_index()
            fig=go.Figure()
            fig.add_trace(go.Scatter(x=pc['timestamp'],y=pc['provisioned_capacity'],
                name='Provisioned Capacity',line=dict(color='#a371f7',width=2)))
            wt=df.set_index('timestamp')['usage_units'].resample('W').mean().reset_index()
            fig.add_trace(go.Scatter(x=wt['timestamp'],y=wt['usage_units'],
                name='Actual Usage',line=dict(color='#0078d4',width=2,dash='dot')))
            fig.update_layout(**PLOTLY_THEME,height=380,
                title=dict(text="Provisioned Capacity vs Actual Usage (Weekly)",font=dict(size=13,color='#c8d8f0')))
            st.plotly_chart(fig,use_container_width=True)

    # Cost vs Usage scatter by region
    if 'cost_usd' in df.columns:
        fig=px.scatter(df,x='usage_units',y='cost_usd',
        color='region' if 'region' in df.columns else None,
        color_discrete_sequence=list(COLORS.values()),
        opacity=0.4,
        labels={'usage_units':'Usage Units','cost_usd':'Cost (USD)'})
        fig.update_layout(**PLOTLY_THEME,height=380,
            title=dict(text="Cost (USD) vs Usage Units — by Region",font=dict(size=13,color='#c8d8f0')))
        st.plotly_chart(fig,use_container_width=True)

# ══════════════════════════════════════════
# TAB 7 — economic_growth_index, marketing_index, it_spending_growth
# ══════════════════════════════════════════
with tab7:
    ext = ['economic_growth_index', 'marketing_index', 'it_spending_growth']
    prs = [c for c in ext if c in df.columns]
    ecl = ['#a371f7', '#f78166', '#ffa657']

    if not prs:
        st.warning("External indicator columns not found.")
    else:
        if 'timestamp' in df.columns:
            agg3 = st.selectbox("Aggregate by", ["Daily","Weekly","Monthly"], key="t7_agg_unique")
            fr3  = {"Daily":"D","Weekly":"W","Monthly":"M"}[agg3]
            fig  = go.Figure()
            for i, c in enumerate(prs):
                ts3 = df.set_index('timestamp')[c].resample(fr3).mean().reset_index()
                fig.add_trace(go.Scatter(
                    x=ts3['timestamp'], y=ts3[c],
                    name=c, line=dict(color=ecl[i], width=2)
                ))
            fig.update_layout(**PLOTLY_THEME, height=340, hovermode='x unified',
                title=dict(text=f"External Indicators Over Time ({agg3})", font=dict(size=13, color='#c8d8f0')))
            st.plotly_chart(fig, use_container_width=True)

        # Correlation scatter — loop is INSIDE with tab7, NOT indented further
        st.markdown("**Correlation: Each External Indicator vs Usage Units**")
        corr_cols = st.columns(len(prs))

        for i, (c, col) in enumerate(zip(prs, corr_cols)):
            with col:
                corr = df[['usage_units', c]].dropna().corr().iloc[0, 1]
                fig  = go.Figure(go.Scatter(
                    x=df[c], y=df['usage_units'],
                    mode='markers',
                    marker=dict(color=ecl[i], opacity=0.35, size=4)
                ))
                fig.update_layout(**PLOTLY_THEME, height=260,
                    title=dict(text=f"{c}<br><sup>Correlation: {corr:.3f}</sup>",
                               font=dict(size=11, color='#c8d8f0')))
                fig.update_xaxes(title=c)
                fig.update_yaxes(title='Usage Units')
                st.plotly_chart(fig, use_container_width=True)

        # Box plot is OUTSIDE the for loop — runs only ONCE, no duplicate key
        if 'region' in df.columns:
            st.markdown("**Distribution of External Indicators by Region**")
            sel_ext = st.selectbox("Select Indicator", prs, key="ext_box_final")
            fig = px.box(df, x='region', y=sel_ext, color='region', points='outliers')
            fig.update_layout(**PLOTLY_THEME, height=320, showlegend=False,
                title=dict(text=f"{sel_ext} Distribution by Region",
                           font=dict(size=13, color='#c8d8f0')))
            st.plotly_chart(fig, use_container_width=True)
